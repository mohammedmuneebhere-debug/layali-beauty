  ...
  return (
    <section className="py-20 section-glide relative overflow-hidden border-t border-layali-pink/10">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 mb-8">
        <p className="text-xs tracking-[0.28em] uppercase text-layali-pink mb-3">{t.ritual.eyebrow}</p>
        <h2 className="font-serif text-4xl lg:text-5xl font-bold text-white mb-3">{t.ritual.title}</h2>
        <p className="text-white/60 max-w-xl leading-relaxed">{t.ritual.subtitle}</p>
        <p className="mt-6 text-[11px] tracking-[0.22em] uppercase text-white/40">
          III · {t.ritual.drag}
        </p>
      </div>
          ...
          return (
            ...
            >
              <Link href={`/shop/${product.id}`} className="block group">
                ...
                <p className="text-[10px] tracking-[0.2em] uppercase text-layali-pink mb-1">
                  {product.category}
                </p>
                <h3 className="font-serif text-lg text-white line-clamp-2 group-hover:text-layali-pink-light transition-colors">
                  {product.name}
                </h3>
                <p className="font-serif text-xl text-white/90 mt-1">{formatPrice(Number(product.price))}</p>
              </Link>
            </motion.div>
          );
        })}
